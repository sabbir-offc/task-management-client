...
import { updateSingleTask } from "../../api/user";
...
const UpdateTask = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const dbRes = await updateSingleTask(taskInfo, task?._id);
      if (dbRes.acknowledged) toast.success("Task Updated Successfully.");
  ...
  return (
    <div className="p-5">
      <h3 className="text-2xl text-center mb-5 text-[#FF9800] font-semibold">
        Update your <span className="text-[#5F8670]">task</span> info.
      </h3>
      ...
      >
        ...
        >
          {loading ? <Loader className="animate-spin m-auto" /> : "Update Task"}
        </button>
      </form>
    </div>
  );
